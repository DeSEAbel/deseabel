...
class MarineMap:
    ...
    def __init__(self, width: int, height: int, step: int, longitude_west: float, latitude_north: float) -> None:
        """Init function.
    ...
    def compute_distance_matrix(self, x0: float, y0: float) -> 'np.ndarray[Tuple[float, float]]':
    ...
    def compute_decibel_matrix_with_propagation(self, x0: float, y0: float, sound_level: float) -> 'np.ndarray[Tuple[float, float]]':
    ...
    def update_and_add_heatmaps(self, list_noise_impactor: List[NoiseImpactor]):
        ...
        for noise_impactor in list_noise_impactor:
            ...
            self.matrix_decibel += 10 ** (self.compute_decibel_matrix_with_propagation(x0, y0, noise_impactor.sound_level) / 10)
    ...
    def matrix_decibel_to_geopandas(self, matrix_decibel: 'np.ndarray[Tuple[float, float]]') -> geopandas.GeoDataFrame:
    ...
    def quantify_decibel_matrix_by_level(self, marine_fauna: MarineFauna) -> 'np.ndarray[Tuple[int, int]]':
    ...
    def get_coords_xy_from_geopandas(self, gdf: geopandas.GeoDataFrame) -> 'np.ndarray[Tuple[float, float], ("N", 2)]':
